...
import { LogOut, Plus, Search, Vault, Sparkles, Settings, Star, Copy, Check, Eye, EyeOff, ExternalLink } from "lucide-react";
      ...
      <aside className="border-r border-[rgba(168,198,224,0.1)] p-5 flex flex-col justify-between sticky top-0 h-screen">
        ...
        <div className="space-y-3">
          <div className="card-kr p-3 text-xs">
            ...
            <div className="truncate font-medium" data-testid="sidebar-user-email">{user?.email}</div>
          </div>
          <button onClick={onLogout} className="btn-ghost w-full justify-center !py-2 text-xs" data-testid="logout-btn">
...
export function VaultPage() {
  ...
  async function load() {
    ...
    try {
      ...
      toast.error(formatApiErrorDetail(err.response?.data?.detail) || "Error al cargar la bóveda");
  ...
  function onDeleted(id) { setCreds((arr) => arr.filter((a) => a.id !== id)); }
    ...
    <div className="max-w-6xl mx-auto p-6 md:p-10">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-[var(--kript-text-dim)] mt-1">
            {loading ? "Cargando…" : `${creds.length} ${creds.length === 1 ? "entrada" : "entradas"} protegidas`}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={openNew} className="btn-primary" data-testid="new-credential-btn">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input placeholder="Buscar por nombre, usuario o dominio…"
            value={q} onChange={(e) => setQ(e.target.value)}
            className="input-kr pl-10" data-testid="search-input"/>
        </div>
      ...
      {loading ? (
        <div className="font-mono-kr text-sm text-[var(--kript-text-dim)] animate-pulse">CARGANDO BÓVEDA…</div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid gap-px bg-[rgba(168,198,224,0.12)] border border-[rgba(168,198,224,0.12)]">
          {filtered.map((c) => (
            <div key={c.id} className="bg-[var(--kript-surface)] p-5 hover:bg-[var(--kript-surface-2)] transition group" data-testid={`credential-row-${c.id}`}>
              <div className="flex items-start justify-between gap-4">
                <button onClick={() => openEdit(c)} className="text-left flex-1 min-w-0" data-testid={`open-credential-${c.id}`}>
                  <div className="flex items-center gap-2">
                    {c.favorite && <Star size={14} className="text-[var(--kript-primary)]" fill="currentColor" />}
                    <div className="font-heading font-semibold text-lg truncate">{c.name}</div>
                    <span className="text-[10px] font-mono-kr tracking-widest text-[var(--kript-secondary)] uppercase border border-[rgba(168,198,224,0.2)] px-2 py-0.5">{c.category}</span>
                  </div>
                  <div className="text-sm text-[var(--kript-text-dim)] mt-1 truncate">{c.username || "—"}</div>
                </button>

                <div className="flex items-center gap-1">
                  <button onClick={() => setRevealed((r) => ({ ...r, [c.id]: !r[c.id] }))} className="p-2 text-[var(--kript-text-dim)] hover:text-[var(--kript-primary)]" title="Revelar" data-testid={`reveal-${c.id}`}>
                    {revealed[c.id] ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                  <button onClick={() => copyToClipboard(c.username, setCopied, `u-${c.id}`)} className="p-2 text-[var(--kript-text-dim)] hover:text-[var(--kript-primary)]" title="Copiar usuario" data-testid={`copy-user-${c.id}`}>
                    {copied === `u-${c.id}` ? <Check size={16} className="text-[var(--kript-primary)]" /> : <Copy size={16} />}
                  </button>
                  <button onClick={() => copyToClipboard(c.password, setCopied, `p-${c.id}`)} className="p-2 text-[var(--kript-primary)] hover:text-[var(--kript-primary-strong)]" title="Copiar contraseña" data-testid={`copy-pass-${c.id}`}>
                    {copied === `p-${c.id}` ? <Check size={16} /> : <Copy size={16} />}
                  </button>
                  {c.url && (
                    <a href={c.url.startsWith("http") ? c.url : `https://${c.url}`} target="_blank" rel="noreferrer" className="p-2 text-[var(--kript-text-dim)] hover:text-[var(--kript-primary)]" title="Abrir URL" data-testid={`open-url-${c.id}`}>
              ...
              {revealed[c.id] && (
                <div className="mt-3 font-mono-kr text-sm bg-[#05090f] border border-[rgba(198,224,138,0.25)] p-3 text-[var(--kript-primary)] break-all" data-testid={`password-view-${c.id}`}>
      ...
      <CredentialModal open={modalOpen} onClose={() => setModalOpen(false)}
        initial={editing} onSaved={onSaved} onDeleted={onDeleted}/>
    </div>
  );
}
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 md:p-10">
      <div className="mb-6">
        ...
        <p className="text-[var(--kript-text-dim)] mt-1">Contraseñas fuertes generadas con CSPRNG del lado del servidor.</p>
      </div>
      {/* Reuse the component */}
      <PasswordGenerator />
    </div>
