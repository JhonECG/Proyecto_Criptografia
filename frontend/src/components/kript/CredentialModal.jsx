...
// onSaved receives the complete credential object (new or updated).
// onDeleted receives the credential id.
// Callers (VaultPage) are responsible for dispatching to VaultContext.
export default function CredentialModal({ open, onClose, initial, onSaved, onDeleted }) {
  ...
  async function save(e) {
    ...
    try {
      ...
      const item = isEdit ? { ...initial, ...payload } : payload;
      await onSaved?.(item);
        ...
        <div className="flex items-center justify-between p-5 border-b border-[rgba(168,198,224,0.1)]">
          <div>
            ...
            <div className="font-display font-black text-xl mt-1">
              {isEdit ? form.name || "Credencial" : "Agregar al vault"}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 border ${
                form.favorite
                  ? "border-[var(--kript-primary)] text-[var(--kript-primary)]"
                  : "border-[rgba(168,198,224,0.2)] text-[var(--kript-text-dim)]"
              } hover:text-[var(--kript-primary)]`}
            ...
            <button
              onClick={onClose}
              className="p-2 border border-[rgba(168,198,224,0.2)] text-[var(--kript-text-dim)] hover:text-white"
        ...
        <form onSubmit={save} className="p-5 grid gap-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            <label className="label-kr flex items-center justify-between">
              ...
              <button
                ...
                className="text-[var(--kript-primary)] text-xs hover:underline inline-flex items-center gap-1"
            ...
            <div className="relative">
              <input
                ...
                className="input-kr font-mono-kr pr-20"
                value={form.password}
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => setShow((s) => !s)}
                  className="p-1.5 text-[var(--kript-text-dim)] hover:text-[var(--kript-primary)]"
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[rgba(168,198,224,0.1)]">
            <div>
              {isEdit && (
                <button
                  type="button"
                  onClick={remove}
                  className="inline-flex items-center gap-2 text-sm text-[var(--kript-danger)] hover:underline"
            ...
            <div className="flex items-center gap-3">
              <button type="button" className="btn-ghost" onClick={onClose} data-testid="cancel-credential">
                Cancelar
              </button>
