  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-[var(--kript-bg)]">
      ...
      <div className="flex flex-col justify-center p-8 md:p-14 relative">
        <Link to="/" className="absolute top-6 left-6 text-sm text-[var(--kript-text-dim)] hover:text-[var(--kript-primary)] inline-flex items-center gap-2" data-testid="back-to-landing-r">
        ...
        <div className="max-w-md w-full">
          ...
          <form onSubmit={onSubmit} className="mt-10 space-y-5" data-testid="register-form">
            <div>
              ...
              <input type="text" className="input-kr" placeholder="Opcional"
                value={name} onChange={(e) => setName(e.target.value)} data-testid="register-name-input"/>
            </div>
            <div>
              <label className="label-kr">Correo</label>
              <input type="email" required className="input-kr" placeholder="tú@correo.com"
                value={email} onChange={(e) => setEmail(e.target.value)} data-testid="register-email-input"/>
            </div>
            <div>
              <label className="label-kr">Contraseña maestra</label>
              <div className="relative">
                <input type={show ? "text" : "password"} required minLength={8}
                  className="input-kr pr-11 font-mono-kr" placeholder="Mínimo 8 caracteres"
                  value={password} onChange={(e) => setPassword(e.target.value)} data-testid="register-password-input"/>
            ...
            <div>
              ...
              <input type={show ? "text" : "password"} required className="input-kr font-mono-kr"
                value={confirm} onChange={(e) => setConfirm(e.target.value)} data-testid="register-confirm-input"/>
            </div>

            <label className="flex items-start gap-3 p-3 border border-[rgba(255,77,79,0.3)] bg-[rgba(255,77,79,0.05)] text-sm cursor-pointer" data-testid="register-ack-label">
              <input type="checkbox" checked={ack} onChange={(e) => setAck(e.target.checked)}
                className="mt-1 accent-[var(--kript-danger)]" data-testid="register-ack-checkbox"/>
            ...
            {err && <div className="text-sm text-[var(--kript-danger)] font-mono-kr" data-testid="register-error">{err}</div>}

            <button type="submit" className="btn-primary w-full justify-center" disabled={loading} data-testid="register-submit">
              <UserPlus size={18} /> {loading ? "Creando…" : "Crear mi bóveda"}
            </button>
          </form>
